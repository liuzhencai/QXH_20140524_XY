...
@implementation UIView (DWCategory)
...
@implementation UIView (setFrameQuick)
...
{
    [self setOriginY:self.frame.origin.y - decY];
}
...
{
    [self setSizeHeight:self.frame.size.height-decHeigth];
}
...
- (UIImageView *)addImageViewWithFrame:(CGRect)frame imageName:(UIImage *)imageName
{
    ...
    if (imageName) {
        imageView.image = imageName;//[UIImage imageNamed:imageName];
    }
